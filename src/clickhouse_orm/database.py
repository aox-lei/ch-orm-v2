...
class Database:
    ...
    def count(self, model_class: Optional[type[MODEL]], sql: str) -> int:
        """
        ...
        """
        # 使用_substitute方法处理SQL语句，传入model_class参数
        query = self._substitute(sql, model_class)
        # 发送处理后的SQL查询
        r = self._send(query)
    ...
    def select(
        ...
        result_model_class: Optional[type] = None,
            ...
            for line in lines:
                ...
                if line.strip():
                    yield model_class.from_tsv(
                        line, field_names, result_model_class, self.server_timezone, self
                    )
        except StopIteration:
            return
        finally:
