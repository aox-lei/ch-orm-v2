...
class Field(FunctionOperatorsMixin):
    ...
    parent: type["Model"]  # this is set by the parent model
    ...
    alias_name: Optional[str] = None  # this is set by the parent model
    ...
    def column_as(self, alias: str):
        ...
        return f"{self.parent.table_name()}.{self.name} AS {self.alias_name}"

    def desc(self):
        return f"{str(self)} DESC"

    def asc(self):
        return f"{str(self)} ASC"
...
class FixedStringField(StringField):
    ...
    def to_python(self, value) -> str:
        value = super(FixedStringField, self).to_python(value)
...
class TupleField(Field):
    ...
    def to_python(self, value) -> tuple:
        if isinstance(value, str):
            value = parse_array(value)
            value = (self.inner_fields[i].to_python(v) for i, v in enumerate(value))
        elif isinstance(value, bytes):
            value = parse_array(value.decode("UTF-8"))
            value = (self.inner_fields[i].to_python(v) for i, v in enumerate(value))
        elif not isinstance(value, (list, tuple)):
            raise ValueError("TupleField expects list or tuple, not %s" % type(value))
        return self.tuple_field(*[self.inner_fields[i].to_python(v) for i, v in enumerate(value)])
...
class IPv6Field(Field):
    ...
    def to_python(self, value) -> IPv6Address:
...
class MapField(Field):
    ...
    def to_python(self, value) -> dict:
        ...
        value = {self.key.to_python(k): self.value.to_python(v) for k, v in value.items()}
        return value
