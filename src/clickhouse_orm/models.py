...
from typing import TypeVar, Optional, TYPE_CHECKING, Any, Union
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        for n, f in fields:
            if f.alias or f.materialized:
                ...
                defaults[n] = f.to_python(f.default)
...
class Model(metaclass=ModelBase):
    ...
    @classmethod
    def from_tsv(
        cls, line, field_names, result_model_class=None, timezone_in_use=pytz.utc, database=None
    ):
        """
        ...
        for name in field_names:
            if name in cls._fields:
                ...
                kwargs[name] = field.to_python(next(values))
            else:
                kwargs[name] = next(values)

        obj = result_model_class(**kwargs)
