...
class Operator:
    ...
    def _value_to_sql(self, field, value, quote=True):
        ...
        if isinstance(value, F):
            return value.to_sql()
        return field.to_db_string(field.to_python(value), quote)
...
class QuerySet(Generic[MODEL]):
    ...
    def __init__(self, model_cls: type[MODEL], database: Database):
        ...
        self._order_by = []
        self._result_model_class = model_cls
        self._where_q = Q()
        self._prewhere_q = Q()
        self._grouping_fields = []
        self._grouping_with_totals = False
        self._fields = [str(field) for field in model_cls.fields().values()]
    ...
    def __iter__(self) -> Iterator[Any]:
        ...
        """
        for val in self._database.select(self.as_sql(), self._model_cls, self._result_model_class):
            yield val
    ...
    def group_by(self, *fields_or_expr):
        for field in fields_or_expr:
            self._grouping_fields.append(field)
        return self
    ...
    def as_sql(self) -> str:
        ...
        if self._joins:
            for _join_type, _table_name, _on in self._joins:
                _join.append(f"{_join_type} {_table_name} ON {_on.to_sql(self._model_cls)}")
        join_str = " ".join(_join)
    ...
    def count(self) -> int:
        ...
        """
        if self._distinct or self._limits or self._grouping_fields:
        ...
        if self._grouping_fields:
            ...
            if self._grouping_with_totals:
                sql += " WITH TOTALS"
        return self._database.count(self._model_cls, sql)
